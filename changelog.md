...
### Fixed
- The tray no longer goes silently missing when the extension is disabled and re-enabled in quick succession, which the shell does on every lock and unlock. `unown_name()` reports nothing when it completes, so the outgoing instance's release could land after the incoming instance's request, on the shell's single bus connection, taking `org.kde.StatusNotifierWatcher` straight back off the new instance. What was left looked like a working extension but owned no watcher at all, so no app could register with it until it was toggled by hand, and nothing in the code noticed. The name is now watched and reclaimed whenever it falls free.
- An app registering an item the tray already holds now gets that item rebuilt rather than having the call ignored. Apps re-register precisely when they have torn down and recreated their item, so treating it as a duplicate left the tray talking to something that no longer existed. Items found by the tray's own sweep are still skipped when already known, since those are not re-registrations.
  - Both of the above were found by reading the AppIndicator and KStatusNotifierItem Support extension, which handles both cases; no code was taken from it.
- Tray icons now appear for apps that start before the extension does. At login gnome-session launches autostart apps as soon as `org.gnome.Shell` is on the bus, which is before extensions are enabled, so apps that start with the session — Discord, Dropbox and KeePassXC in the report — call `RegisterStatusNotifierItem` before there is a watcher to answer, and most never try again. Recovering from that is the tray's job, and the sweep meant to do it had three holes. It ran once, at the moment the watcher was created, when a cold-starting app may not have exported its item yet. It probed connections one at a time with a one-second timeout, so a single unresponsive service could stall the whole sweep past the window it was there to cover, and a probe that timed out was discarded silently. And it only ever looked at `/StatusNotifierItem`, so any app using a different object path was invisible to it. The sweep now runs at startup and again at 2, 5, 15, 30 and 60 seconds, probes every connection in parallel with a three-second timeout, and walks a connection's object tree when nothing is at the default path — which is how apps built on libayatana-appindicator, whose items sit at `/org/ayatana/NotificationItem/<id>`, are found. Separately, an app taking a well-known item name — `org.kde.StatusNotifierItem-*` as the spec has it, or the `org.freedesktop.` variant Electron apps use — is now a cue to go looking for its item, so one that starts long after login and fails to register is still picked up. The watcher also claims its bus name as the first thing the extension does on enable, rather than after the settings, signal handlers and keyboard shortcut are wired up — the shell enables extensions one after another, so anything done ahead of that is time an app starting with the session might spend looking for a tray that isn't there yet. Thanks to [@MatthewFallon](https://github.com/MatthewFallon) for the report (#28).
